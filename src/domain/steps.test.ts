...
import { seedProjects, seedWorkspaces } from '../data/fixtures';
...
describe('inputsHash', () => {
  it('は同じ入力で同じ値になる', () => {
    expect(inputsHash('brand', project, workspace)).toBe(
      inputsHash('brand', project, workspace),
    );
  });

  it('は入力が変わると変わる', () => {
    const changed = { ...project, brandConcept: `${project.brandConcept}（改）` };
    expect(inputsHash('brand', changed, workspace)).not.toBe(
      inputsHash('brand', project, workspace),
    );
  });
  ...
  it('は下流の変更に影響されない（上流ステップの入力は下流を含まない）', () => {
    ...
    expect(inputsHash('brand', project, withMoreShots)).toBe(
      inputsHash('brand', project, workspace),
    );
  });
});
