...
- 提案書の章本文も章ごとに1フィールドの生成物。生成→差分プレビュー→適用を通り、章単位で編集・保護できる。Markdown は実ファイルとしてダウンロードでき、画像には出自（AI生成／持ち込み／外部参照）が併記される。
...
- 実ファイル出力は Markdown のみ。PDF／PowerPoint／Word はファイル名と履歴だけで、実生成はサーバー側に接続してから。
...
src/
  ...
  domain/     ステップ契約、フィールド定義、provenance、差分の算出と適用
